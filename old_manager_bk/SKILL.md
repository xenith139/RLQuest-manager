...
- **"manage"** (also **"mng"** or **"m"**) — when the user says any of these words (with no other arguments), the manager runs the **OODA-Constraints Loop** (see `process_framework.md`):
  ...
  **3b. VALIDATE** (operational gate — MANDATORY before ACT):
  - Run every recommended action through the operational checklists in `process_framework.md` §3b
  - Data pipeline task? → smoke test + CPU + memory + throughput all validated?
  - Training task? → unit test + GPU util + AMP + checkpoint + design doc?
  - Implementation task? → design doc complete?
  - All actions → chained to follow-up? success/failure criteria?
  - **If any check fails → modify the recommendation to include the missing validation step FIRST**
  - The manager must enforce the same rules on its own recommendations that it enforces on dev
...
- **"send"** (also **"s"**) — when the user says "send" or "s":
  ...
  **Phase 4: Post-Completion ORIENT (CRITICAL — do NOT skip)**
  - When dev finishes the prompted task and returns to idle, **immediately run one more ORIENT cycle**:
    - Are there idle resources? (GPU, CPU, dev)
    - Is there available work that could use those resources?
    - If yes → formulate and send the next task immediately. Do NOT wait for user to invoke `/m` again.
  - This prevents the failure mode where dev sits idle for hours between manage cycles.
  - **Always chain tasks in prompts**: "After X passes, immediately proceed to Y with nohup. Then start Z."
  - Never end a prompt with "report when ready" without including the next step.
...
- **"linear"** (also **"l"**) — runs the linear question analysis from `manager_linear.md`:
  1. Read `manager_linear.md` — the 38-question sequential checklist
  2. Answer **every question in order**, one by one. No skipping. Each answer must include evidence (filesystem check, process check, metric, or document reference).
  3. Use lightweight bash commands to gather evidence for each question (ls, ps aux, nvidia-smi, tail logs, cat status.json, etc.)
  4. Write the complete Q&A analysis + resulting plan and prompt to `/home/ubuntu/workspace/RLQuest-manager/m_linear.md`
  5. Do NOT send anything to dev. Do NOT write to `manager.md`. Only output to `m_linear.md`.
  6. The output file `m_linear.md` should contain:
     - Each question number and text
     - The answer with evidence
     - A "## Plan" section at the end summarizing the decided action(s)
     - A "## Recommended Prompt" section with the ready-to-send prompt for dev
  7. This mode is analysis-only — the user reviews `m_linear.md` and decides whether to send.

- **"all"** — starts the autonomous manager loop using `/loop`:
  1. Run the first `/m auto` cycle immediately.
  2. After the first cycle completes, use the `CronCreate` tool to schedule `/m auto` on a recurring interval.
  3. Default interval: 10 minutes. User can specify: `/m all 5m`, `/m all 30m`.
  4. Report to user: "Manager loop started. Running /m auto every [interval]. Cancel with /m stop."

- **"auto"** — one full autonomous cycle (used by the loop, can also be called manually):
  1. Run the full **manage** workflow (OBSERVE → ORIENT → DECIDE → VALIDATE → ACT → LEARN)
  2. If `manager.md` contains a recommendation:
     a. Automatically execute the **send** workflow (send prompt to dev, monitor, intervene)
     b. After dev completes, run Phase 4 post-completion ORIENT
     c. If more immediate work → send next task (chain tasks)
     d. Repeat until dev is idle with no immediate work
  3. If manager decides no action is needed (dev is busy, everything on track): report "loop idle — no action needed" and return.

  **Auto-send rules** — when to proceed vs pause:
  ...
  - Manager confidence is MEDIUM or higher

  PAUSE AND REPORT (cancel loop, wait for user):
  - Strategic pivot needed (e.g., switch from V4 to V5)
  - Constraint changed fundamentally
  - Dev encountered an unrecoverable error
  - Cost of next action is very high (>24 hours GPU time without prior validation)
  - Manager confidence is LOW
  - To pause: use `CronDelete` to cancel the scheduled loop and report to user

- **"stop"** — stops the autonomous manager loop:
  1. Use `CronDelete` to remove the scheduled `/m auto` task.
  2. Report to user: "Manager autonomous loop stopped."

- **"loop-status"** — check loop state:
  1. Use `CronList` to show active scheduled tasks.
  2. Report: active/inactive, interval, next scheduled run.
...
1. **OBSERVE**: Check dev session (Steps 1-4). Verify ground truth (ls, ps, nvidia-smi, tail logs).
2. **ORIENT**: Gap analysis → constraint ID → hypothesis → enumerate actions → ETG → priority rules.
3. **DECIDE**: Choose highest expected value action(s). Strategic analysis overrides the task list.
3b. **VALIDATE**: Run every recommendation through operational checklists (data pipeline: smoke test + perf? training: unit test + GPU util? implementation: design doc?). If any check fails → add the validation step to the prompt BEFORE the action. Manager enforces rules on itself.
4. **ACT**: Write `manager.md` (constraint, hypothesis, ETG, validation checklist, recommendation, success criteria). Chain follow-up tasks — never leave dev idle.
5. **LEARN**: Update `goal_tracker.md`. Update process if a gap was found.
